from .vmamba_vmunetv2 import VSSM
...
class VMUNetV2(nn.Module):
    ...
    def forward(self, sample):
        ...
        if self.deep_supervision:
            ...
            loss = self.loss_fn(out_0 + out_1, mask)
            return {'prediction': out_0 + out_1, 'loss': loss}
            # return out_0 + out_1
            # return torch.sigmoid(out_0 + out_1)  # [2, 1, 256, 256]
        else:
            loss = self.loss_fn(seg_outs[-1], mask)
            return {'prediction': seg_outs[-1], 'loss': loss}
